...
void main() {
  ...
  if(fract(uv.x / gridRpeat) < gridWidth || fract(uv.y / gridRpeat) < gridWidth){
    gl_FragColor += vec4(1.0,1.0,1.0,fade);
  }else{
    float fade = clamp(snoise3((floor(uv.xyx*10.0)/10.0+0.328)+vec3(0,0,time*.1))-0.4,0.0,1.0);
    gl_FragColor = vec4(fade,fade,fade,0.05);
  }
